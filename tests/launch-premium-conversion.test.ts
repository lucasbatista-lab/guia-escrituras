...
describe("launch premium conversion", () => {
  it("home hero keeps a free primary CTA and product proof without inventing scarcity", () => {
  ...
  it("plans stay honest: Conversar=Essencial, Dia1 preview, Caminho full", () => {
  ...
  it("authenticated entry and support address remain clear", () => {
    const living = read("src", "components", "inicio", "inicio-living.tsx");
    expect(living).toContain("InicioLiving");
    expect(living.length).toBeGreaterThan(200);
