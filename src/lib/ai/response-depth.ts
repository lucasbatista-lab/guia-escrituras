...
export function getResponseDepthGuidance(
  ...
  const promptLines = [
    ...
    ...depthSpecific,
    "- Estruture com naturalidade: acolhimento específico → reflexão à luz da passagem → um passo prático. Pergunta só se faltar um dado essencial (no máximo uma).",
    "- Não inicie repetindo que é uma IA. A interface já mostra isso.",
    "- Evite tom de artigo genérico ou lista de ‘pontos cristãos’. Fale da situação concreta.",
